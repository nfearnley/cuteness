...
class BadMultilineCommand(CommandError):
...
async def process_commands(self, message):
    ...
    # If at least on of the lines is a multi-line command (these are only allowed as the first command)
    ...
    if multiline_command:
        ...
        await multiline_command.command.dispatch_error(multiline_command, BadMultilineCommand(f"{username} tried to run a multi-line command in the middle of a sequence."))
        return
